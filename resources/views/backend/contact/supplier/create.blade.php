<div class="modal-content">
    <form class="needs-validation" id="supplier_add_form" action="{{ route('backend.supplier.store') }}" method="Post" enctype="multipart/form-data">
        @method('POST')
        @csrf
        <div class="modal-header">
            <h4 class="title" id=""> Supplier Add </h4>
        </div>
        ...
        <div class="modal-footer">
            <button type="submit" class="btn btn-primary save_supplier_button">SAVE</button>
...
<script >
    $(document).on('submit', 'form#supplier_add_form', function(e) {
        e.preventDefault();
        var supplierForm = $("form#supplier_add_form");
        var formData = supplierForm.serialize();
        $('.save_supplier_button').attr('disabled',true);
        $.ajaxSetup({
            headers: {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (myObject.hasOwnProperty(key)) {
                    ...
                    $("form#supplier_add_form input[name='" + key + "']").after("<div class='text-danger'><strong>" + ' ' + " </strong></div>");
                    $("form#supplier_add_form input[name='" + key + "']").after("<div class='text-danger'><strong>" + myObject[key] + " </strong></div>");

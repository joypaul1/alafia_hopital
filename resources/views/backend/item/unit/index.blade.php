...
{{-- @include('backend._partials.page_header', [
    ...
    'name' => 'Create Unit',
    'route' => route('backend.itemconfig.unit.create')
 ]) --}}
        ...
        <div class="card">
            <div class="body">
                <div class="table-responsive">
                    <table ellspacing='0' class="table table-bordered text-center dataTable unit_table">
                        <thead>
                            <tr>
                                <th> Sl.</th>
                                <th> Name</th>
...
<script>
    ...
    $(document).on('submit', 'form#unit_edit_form', function(e) {
        e.preventDefault();

        var registerForm = $("form#unit_edit_form");
        var formData = registerForm.serialize();
        $('.edit_unit_button').attr('disabled',true);
        $.ajax({
    ...
    $(document).on('submit', 'form#unit_add_form', function(e) {
        ...
        var registerForm = $("form#unit_add_form");
        var formData = registerForm.serialize();
        $('.save_unit_button').attr('disabled',true);
        $.ajax({
            enctype: 'multipart/form-data',
            url: $(this).attr('action'),
            type: 'POST',

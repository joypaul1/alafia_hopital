    ...
    <form class="needs-validation" id="labTestTube_add_form" action="{{ route('backend.siteConfig.labTestTube.store') }}" method="Post" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            <div class="form-validation">
                ...
                <div class="form-group">
                    @include('components.backend.forms.select2.option',['label'=>'Service Type', 'name' =>'labTestTube_type_id', 'optionDatas'=>$type , 'required'=>true])
                    @include('components.backend.forms.input.errorMessage', ['message' =>$errors->first('labTestTube_type_id')])
                </div>
                <div class="form-group">
                    @include('components.backend.forms.input.input-type',[ 'name' => 'labTestTube_price', 'placeholder' => 'price will be here...', 'required'=>true ])

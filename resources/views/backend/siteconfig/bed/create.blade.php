...
<div class="modal-content">
    <form class="needs-validation" id="bed_add_form" action="{{ route('backend.itemconfig.bed.store') }}" method="Post"
        ...
        <div class="modal-body">
            <div class="form-validation">
                ...
                <div class="form-group">
                    @include('components.backend.forms.select2.option',['label'=>'Bed Group', 'name' =>'bed_group_id', 'optionDatas'=>$categories, 'required'=> true ])
                    @include('components.backend.forms.input.errorMessage', ['message' =>$errors->first('bed_group_id')])
                </div>
                <div class="form-group">
                    @include('components.backend.forms.select2.option',['label'=>'Bed Type', 'name' =>'bed_type_id', 'optionDatas'=>$categories, 'required'=> true ])
                    @include('components.backend.forms.input.errorMessage', ['message' =>$errors->first('bed_type_id')])
                </div>
                <div class="form-group">
                    @include('components.backend.forms.select2.option',['label'=>'Cabin', 'name' =>'cabin_id', 'optionDatas'=>$categories, 'required'=> true ])
                    @include('components.backend.forms.input.errorMessage', ['message' =>$errors->first('cabin_id')])
                </div>
                <div class="form-group">
                    @include('components.backend.forms.select2.option',['label'=>'Ward', 'name' =>'ward_id', 'optionDatas'=>$categories, 'required'=> true ])
